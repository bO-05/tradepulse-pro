# QA-10 (round 4) independent HTTP contract matrix (read-only; curl-based).
# Charter items: /api/health; /api/no-such GET+POST; /agentmail/no-such GET+POST;
# /agentmail/webhook GET, POST-no-svix; /llms.txt; /dashboard;
# /specs/26_00_00_Electrical_Systems_Spec.pdf; /specs/missing.pdf.
# Usage: powershell -ExecutionPolicy Bypass -File scripts/qa-rem/qa10-http-contract.ps1
param(
...
$tmp = Join-Path $env:TEMP ("qa10-" + [guid]::NewGuid().ToString("N"))
...
function Probe {
  ...
  if ($ExpectNotStatus -and "$status" -eq "$ExpectNotStatus") { $pass = $false; $reasons += "status=$status must-not-be=$ExpectNotStatus" }
  ...
  $dest = Join-Path $EvidenceDir ("remediation-qa10-raw-" + $safeName + ".txt")
  $rawDump = "URL: $Method $Url`nSTATUS: $status`nCONTENT-TYPE: $ctype`nSIZE: $size`nSHA256: $sha`nHEADERS:`n$headers`nBODY(first 2000 chars):`n" + $bodyText.Substring(0, [Math]::Min(2000, $bodyText.Length))
...
Add-Line "=== QA-10 HTTP CONTRACT MATRIX (independent round-4 re-check, curl) ==="
...
$results += Probe -Name "api-no-such-post" -Method POST -Url "$BaseUrl/api/no-such" -Body '{"probe":"qa10"}' -ExpectStatus 404 -ExpectTypeLike "application/json" -ExpectBodyContains '"error"'
...
$results += Probe -Name "agentmail-no-such-post" -Method POST -Url "$BaseUrl/agentmail/no-such" -Body '{"probe":"qa10"}' -ExpectStatus 404 -ExpectTypeLike "application/json" -ExpectBodyContains '"error"'
$results += Probe -Name "webhook-get" -Method GET -Url "$BaseUrl/agentmail/webhook" -ExpectStatus 200 -ExpectTypeLike "application/json" -ExpectBodyContains '"status"'
$results += Probe -Name "webhook-post-no-svix" -Method POST -Url "$BaseUrl/agentmail/webhook" -Body '{"probe":"qa10"}' -ExpectNotStatus 200 -ExpectTypeLike "application/json" -ExpectBodyContains '"error"'
$results += Probe -Name "llms-txt" -Method GET -Url "$BaseUrl/llms.txt" -ExpectStatus 200 -ExpectBodyContains "TradePulse Pro"
...
$dest = Join-Path $EvidenceDir "remediation-qa10-http-contract.txt"
