...
export const PREFIX = "AUDIT-QA19-";
...
export function writeEvidence(name, obj) {
  ...
  const file = path.join(EVIDENCE_DIR, `fix4-qa19-${name}.json`);
...
export function readEvidence(name) {
  return JSON.parse(fs.readFileSync(path.join(EVIDENCE_DIR, `fix4-qa19-${name}.json`), "utf8"));
}

export function writeLog(name, lines) {
  fs.mkdirSync(EVIDENCE_DIR, { recursive: true });
  const file = path.join(EVIDENCE_DIR, `fix4-qa19-${name}.log`);
...
export async function hardDeleteProject(c, title) {
  ...
  if (!String(existing.title).startsWith(PREFIX)) {
    return { deleted: false, error: `refusing to delete non-QA19 project ${existing.title}` };
  }
    ...
    if (a.status === "executed") {
      try {
        await c.mutation("agreements:voidExecutedAgreement", {
          agreementId: a._id,
          reason: "QA19 cleanup: void recorded execution before AUDIT-QA19 fixture teardown.",
        });
      } catch (err) {
...
export async function projectSnapshot(c, projectId) {
  const [project, packages, agreements, bids, contractors, logs] = await Promise.all([
    ...
    c.query("auditLogs:listRecentLogs", { projectId, limit: 400 }),
  ]);
  return {
    project,
    packages: packages || [],
