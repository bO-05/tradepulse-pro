/**
 * QA37 shared harness (fix4-qa37-* evidence).
 *
 * Marker-aware like qa36-lib: FIX-NEW-75/76 mints credit rows as
 *   `Cross-Trade Clash Credit [<clashId>]: Deduct redundant <title>`
 * while legacy rows are `Cross-Trade Clash Credit: Deduct redundant <title>`.
 * Matches BOTH formats and carries the clash id through every invariant.
 */
...
export const PREFIX = "AUDIT-QA37-";
...
export function writeEvidence(name, obj) {
  ...
  const file = path.join(EVIDENCE_DIR, `fix4-qa37-${name}.json`);
...
export function readEvidence(name) {
  return JSON.parse(fs.readFileSync(path.join(EVIDENCE_DIR, `fix4-qa37-${name}.json`), "utf8"));
}

export function writeLog(name, lines) {
  fs.mkdirSync(EVIDENCE_DIR, { recursive: true });
  const file = path.join(EVIDENCE_DIR, `fix4-qa37-${name}.log`);
