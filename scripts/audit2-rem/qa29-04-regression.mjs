/**
 * QA29-04 core regression (live, one pass) on AUDIT-QA29 fixtures:
 *  - clash detect with no evidence returns zeros (no fabricated cards)
...
import { client, readEvidence, writeEvidence, writeLog, projectSnapshot, sleep, call, EVIDENCE_DIR } from "./qa29-lib.mjs";
...
async function main() {
  ...
  record(
    ...
    "clash detect on a zero-bid two-trade project returns honest zeros (no fabricated clashes)",
    noev.doubleBuys.length === 0 && noev.scopeVoids.length === 0 &&
      noev.summary.activeClashesCount === 0 && noev.summary.totalDoubleBuyExposure === 0 &&
      noev.summary.totalScopeVoidExposure === 0 && noev.summary.netBuyoutExposure === 0,
    { provider: noev.provider, model: noev.model, summary: noev.summary }
  );

  // =========================== F1: RFI pending -> clarified ===========================
  const rfiQuestion = "QA29 regression: confirm the firestop sleeve responsibility at the slab penetration detail before bid submission.";
  const rfiSub = await c.mutation("simulation:submitCustomRfi", {
    tradePackageId: S.p2.id,
    contractorId: S.p2.contractors.one,
    subject: "AUDIT-QA29 REGRESSION RFI live smoke",
  ...
  if (progress?.status === "escalated_to_pm") {
    try {
      await c.mutation("rfq:reviewEscalatedRfi", {
        ...
        status: "clarified",
        reviewNote: "QA29 regression: PM certification to complete pending->clarified.",
      });
      certified = true;
  ...
  const dlDir = path.join(EVIDENCE_DIR, "fix4-qa29-regression-downloads");
  ...
  try {
    ...
    await page.goto(`${BASE}/?project=${S.id}&tab=leveling&qa29=regression`, { waitUntil: "domcontentloaded", timeout: 90000 });
    ...
    await shot(page, "fix4-qa29-inline-confirm.png");
    ...
    await page.evaluate(() => {
      ...
      if (ta) {
        ...
        setter.call(ta, "QA29 DRAFT MARKER");
    ...
    await shot(page, "fix4-qa29-ingest-reopen.png");
    record(
      "REG.modal-reset",
    ...
    await shot(page, "fix4-qa29-deeplink-reload.png");
    record(
      "REG.deeplink-reload",
    ...
    await page.goto(`${BASE}/?project=${S.id}&tab=contracts&qa29=print`, { waitUntil: "domcontentloaded", timeout: 90000 });
    await waitForAppReady(page, 60000);
    await dismissTour(page);
    await delay(2200);
    ...
    await page.goto(`${BASE}/?project=${F.csv.id}&tab=leveling&qa29=csv`, { waitUntil: "domcontentloaded", timeout: 90000 });
    await waitForAppReady(page, 60000);
    await dismissTour(page);
    await delay(1500);
    ...
    await page.goto(`${BASE}/?project=${F.register.id}&tab=contracts&qa29=register`, { waitUntil: "domcontentloaded", timeout: 90000 });
    await waitForAppReady(page, 60000);
    await dismissTour(page);
    await delay(1800);
    ...
    await shot(page, "fix4-qa29-register-superseded.png");
