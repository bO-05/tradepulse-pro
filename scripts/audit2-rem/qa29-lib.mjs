...
export const PREFIX = "AUDIT-QA29-";
...
export function writeEvidence(name, obj) {
  ...
  const file = path.join(EVIDENCE_DIR, `fix4-qa29-${name}.json`);
...
export function readEvidence(name) {
  return JSON.parse(fs.readFileSync(path.join(EVIDENCE_DIR, `fix4-qa29-${name}.json`), "utf8"));
}

export function writeLog(name, lines) {
  fs.mkdirSync(EVIDENCE_DIR, { recursive: true });
  const file = path.join(EVIDENCE_DIR, `fix4-qa29-${name}.log`);
