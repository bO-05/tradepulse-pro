...
const SCAN = () => {
  ...
  for (const el of document.querySelectorAll("*")) {
    ...
    const base = layers.length ? layers[layers.length - 1].a >= 0.999 ? layers[layers.length - 1] : pageBase : pageBase;
    let bg = { ...pageBase };
...
const run = async () => {
  ...
  const out = { url: BASE_URL, viewport: "1440x900", tabs: {} };
  try {
    ...
    for (const [id, label] of tabs) {
      ...
      out.tabs[id] = res;
      console.log(`CONTRAST ${id}: strictViolations=${res.totalViolations} gradientText=${res.gradientTextCount}`);
      res.violations.slice(0, 12).forEach((v) => console.log(`  ${v.ratio}:1 (need ${v.required}) ${v.fontSize}/${v.fontWeight} grad=${v.gradientAncestor} fg=${v.fg} bg=${v.bg} :: ${JSON.stringify(v.text)}`));
    }
    await shot(page, "fix4-qa2-contrast-final.png");
    writeJson("fix4-qa2-03-contrast.json", out);
  } finally {
    await browser.close();
  }
};
