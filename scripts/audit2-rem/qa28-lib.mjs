...
export const PREFIX = "AUDIT-QA28-";
...
export function writeEvidence(name, obj) {
  ...
  const file = path.join(EVIDENCE_DIR, `fix4-qa28-${name}.json`);
...
export function readEvidence(name) {
  return JSON.parse(fs.readFileSync(path.join(EVIDENCE_DIR, `fix4-qa28-${name}.json`), "utf8"));
}

export function writeLog(name, lines) {
  fs.mkdirSync(EVIDENCE_DIR, { recursive: true });
  const file = path.join(EVIDENCE_DIR, `fix4-qa28-${name}.log`);
...
export function summarizeAudit(logs, patterns) {
  return patterns.map((p) => ({
    pattern: p.source,
    rows: logs.filter((l) => p.test(l.title) || p.test(l.description)).map((l) => ({
      title: l.title,
