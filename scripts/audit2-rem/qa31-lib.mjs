...
export const URL = process.env.REM_CONVEX_URL || "https://brainy-skunk-440.convex.cloud";
export const WEBSITE = process.env.REM_BASE_URL || "https://brainy-skunk-440.convex.site";
export const PREFIX = "AUDIT-QA31-";
...
export function writeEvidence(name, obj) {
  ...
  const file = path.join(EVIDENCE_DIR, `fix4-qa31-${name}.json`);
...
export function readEvidence(name) {
  return JSON.parse(fs.readFileSync(path.join(EVIDENCE_DIR, `fix4-qa31-${name}.json`), "utf8"));
}

export function writeLog(name, lines) {
  fs.mkdirSync(EVIDENCE_DIR, { recursive: true });
  const file = path.join(EVIDENCE_DIR, `fix4-qa31-${name}.log`);
