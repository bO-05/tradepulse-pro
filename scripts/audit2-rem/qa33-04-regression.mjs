/**
 * QA33-04 core regression (live, one pass) on AUDIT-QA33 fixtures:
 *  - clash detect with no evidence returns honest zeros (2 pkgs/0 bids, 1 pkg, 0 pkgs)
 *  - F1 RFI: pending persisted before analysis -> clarified, question retained
 *  - executed-contract immutability (backend) + UI inline confirm error
 *  - CSV formula neutralization (download)
 *  - print popup: full contract text in an isolated document
 *  - modal draft reset (ingest reopens clean)
 *  - deep-link package persistence across reload
 *  - superseded register row: read-only badge, no execute dead action
 */
import fs from "node:fs";
import path from "node:path";
import { launchBrowser, attachDiagnostics, waitForAppReady, delay, shot } from "./lib.mjs";
import { client, readEvidence, writeEvidence, writeLog, projectSnapshot, sleep, call, EVIDENCE_DIR } from "./qa33-lib.mjs";
...
async function main() {
  ...
  const zeros = (d) =>
    d.doubleBuys.length === 0 && d.scopeVoids.length === 0 &&
    d.summary.activeClashesCount === 0 && d.summary.totalDoubleBuyExposure === 0 &&
    d.summary.totalScopeVoidExposure === 0 && d.summary.netBuyoutExposure === 0;
  record(
    "REG.detect-no-evidence",
    "clash detect returns honest zeros with no fabricated cards for 2-packages/0-bids, 1-package, and 0-package projects",
    zeros(noev) && zeros(noevOne) && zeros(noevNoPkg) &&
      noev.provider === "Proposal-Evidence-Guard" && noevOne.provider === "Project-Scope-Guard" && noevNoPkg.provider === "Project-Scope-Guard",
  ...
  const rfiQuestion = "QA33 regression: confirm the firestop sleeve responsibility at the slab penetration detail before bid submission.";
  const rfiSub = await c.mutation("simulation:submitCustomRfi", {
    tradePackageId: S.p2.id,
    contractorId: S.p2.contractors.one,
    subject: "AUDIT-QA33 REGRESSION RFI live smoke",
  ...
  if (progress?.status === "escalated_to_pm") {
    try {
      await c.mutation("rfq:reviewEscalatedRfi", {
        ...
        status: "clarified",
        reviewNote: "QA33 regression: PM certification to complete pending->clarified.",
      });
      certified = true;
  ...
  const dlDir = path.join(EVIDENCE_DIR, "fix4-qa33-regression-downloads");
  ...
  try {
    ...
    await page.goto(`${BASE}/?project=${S.id}&tab=leveling&qa33=regression`, { waitUntil: "domcontentloaded", timeout: 90000 });
    ...
    await shot(page, "fix4-qa33-inline-confirm.png");
    ...
    await page.evaluate(() => {
      ...
      if (ta) {
        ...
        setter.call(ta, "QA33 DRAFT MARKER");
    ...
    await shot(page, "fix4-qa33-ingest-reopen.png");
    record(
      "REG.modal-reset",
    ...
    await shot(page, "fix4-qa33-deeplink-reload.png");
    record(
      "REG.deeplink-reload",
    ...
    await page.goto(`${BASE}/?project=${S.id}&tab=contracts&qa33=print`, { waitUntil: "domcontentloaded", timeout: 90000 });
    await waitForAppReady(page, 60000);
    await dismissTour(page);
    await delay(2200);
    ...
    await page.goto(`${BASE}/?project=${F.csv.id}&tab=leveling&qa33=csv`, { waitUntil: "domcontentloaded", timeout: 90000 });
    await waitForAppReady(page, 60000);
    await dismissTour(page);
    await delay(1500);
    ...
    await page.goto(`${BASE}/?project=${F.register.id}&tab=contracts&qa33=register`, { waitUntil: "domcontentloaded", timeout: 90000 });
    await waitForAppReady(page, 60000);
    await dismissTour(page);
    await delay(1800);
    ...
    await shot(page, "fix4-qa33-register-superseded.png");
    ...
    record(
      "REG.superseded-register",
      "superseded register row is read-only (badge shown, no Pending badge, no 'Record Execution Status' dead action) while Inspect Draft stays available",
      chipClick.ok && Boolean(supRowState) && supBadge && !supPending && !supExecute &&
        supRowState.buttons.some((b) => b.includes("Inspect Draft")),
      { chips, chip: supChip, supBadge, supPending, supExecute, buttons: supRowState?.buttons, cells: supRowState?.cells }
    );

    // ----- diagnostics: no unhandled page errors; console errors classified -----
    const appConsoleErrors = diag.consoleLogs.filter((l) => l.type === "error" && !/^\[CONVEX [MQA]\(/.test(l.text));
    record(
      "REG.diagnostics",
      "regression journey: zero page errors; every console error is the Convex client's own '[CONVEX M(...)]' refusal log (no application errors)",
