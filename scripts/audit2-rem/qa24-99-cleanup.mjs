/**
 * QA24-99 cleanup: delete ONLY AUDIT-QA24-* projects (voiding executed
 ...
 */
import { client, writeEvidence, writeLog, sleep } from "./qa24-lib.mjs";
...
out.foreignBefore = before.filter((p) => !p.title.startsWith("AUDIT-QA24-")).map((p) => p.title).sort();

for (const p of before.filter((x) => x.title.startsWith("AUDIT-QA24-"))) {
  ...
  for (const a of agrs.filter((x) => x.status === "executed")) {
    try {
      await c.mutation("agreements:voidExecutedAgreement", {
        agreementId: a._id,
        reason: "QA24 teardown: void executed record before deleting the fixture project.",
      });
...
out.leftovers = after.filter((p) => p.title.startsWith("AUDIT-QA24-")).map((p) => p.title);
out.foreignAfter = after.filter((p) => !p.title.startsWith("AUDIT-QA24-")).map((p) => p.title).sort();
