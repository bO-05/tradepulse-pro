/**
 * QA30-04 full bid-day UI journey on a fresh project (created through the dialogs):
 * create -> Div26 + Div23 packages -> contractors -> bids -> clash scan -> deduct
 * -> assign -> award -> execute -> register filters/search + viewer states
 * -> void -> re-award, reconciling backend vs UI at every step.
 * Adds 375px + 720px (200%) overflow probes and websocket diagnostics.
 * Bids are submitted through the public mutation the ingest UI calls (no deterministic
 * direct-bid UI exists; LLM/BYOK excluded). Project title: AUDIT-QA30-JOURNEY.
 */
...
async function main() {
  ...
  for (const p of ((await c.query("projects:listProjects", {})) || []).filter((x) => x.title === TITLE)) {
    ...
    for (const a of agrs0.filter((x) => x.status === "executed")) {
      try { await c.mutation("agreements:voidExecutedAgreement", { agreementId: a._id, reason: "QA30 journey purge of prior run executed record." }); } catch {}
    }
  ...
  try {
    ...
    await page.goto(`${BASE}/?qa30=journey`, { waitUntil: "domcontentloaded", timeout: 90000 });
    ...
    const npFills = {
      ...
      gc: await setVal(page, 'input[aria-label="General contractor or contracting entity"]', "QA30 Journey GC, LLC"),
      ...
      spec: await setVal(page, 'textarea[placeholder*="Outline high-level trade scopes"]', "Division 26 electrical and Division 23 HVAC scope for the QA30 journey."),
    };
    ...
    await delay(2000);
    record("A30-04.1", "fresh project created through the UI dialog", Boolean(proj) && Object.values(npFills).every(Boolean) && npClick.ok, { id: proj?._id, npFills, npClick });
    ...
    const mk26 = await mkPkgUI("26 00 00", "QA30 Journey Electrical", "900000");
    ...
    const mk23 = await mkPkgUI("23 00 00", "QA30 Journey HVAC", "600000");
    ...
    record("A30-04.2", "Div26 + Div23 packages created through the UI dialog",
    ...
    const ctrGo1 = await addCtr(p26.tradeName, "AUDIT-QA30 Journey Prime", "estimating@qa30-journey-e.invalid", "HI-QA30-JE");
    const ctrGo2 = await addCtr(p23.tradeName, "AUDIT-QA30 Journey Mechanical", "estimating@qa30-journey-m.invalid", "HI-QA30-JM");
    ...
    const ctr26 = (ctrs26 || []).find((x) => /QA30 Journey Prime/.test(x.companyName));
    const ctr23 = (ctrs23 || []).find((x) => /QA30 Journey Mechanical/.test(x.companyName));
    record("A30-04.3", "contractors added through the UI for both packages", ctrGo1.ok && ctrGo2.ok && Boolean(ctr26) && Boolean(ctr23),
    ...
    record("A30-04.4", "bids submitted (backend submitDirectBid; deterministic direct-bid UI does not exist, AI ingest excluded)",
    ...
    record("A30-04.5", "cross-trade pre-state: UI KPI matches backend (buys $50,500, voids $46,500, credits $0, no resolved chips)",
    ...
    record("A30-04.6", "UI clash scan banner reconciles with the backend scan message",
    ...
    record("A30-04.7", "UI 1-click deduct: HVAC bid -$38,500 and card/KPI reconcile (credits $38,500; remaining buys $12,000)",
    ...
    record("A30-04.8", "UI assign scope void: Div26 bid +$28,000, inclusion + overlay assigned, KPI voids fall to $18,500",
    ...
    record("A30-04.9", "UI award generates the agreement; audit says 'generated ... pending external execution' (no executed claim)",
    ...
    record("A30-04.10", "UI execution: executed status; audit says 'Execution status recorded ... external signature verification remains required'; generated viewer showed Pending Execution before, RECORDED • SIGNATURE REQUIRED after",
    ...
    await setSearch(page, "QA30 Journey Prime");
    await delay(900);
    ...
    await delay(900);
    record("A30-04.11", "register (executed): chip 'Execution Status Recorded' 1/1, row status badge, search by agreement #/subcontractor/CSI all return the row, Pending filter is empty",
      regExec.exec === "1/1" &&
    ...
    record("A30-04.12", "UI void: agreement superseded, bid unawarded; viewer flips to Superseded; audit says the recorded execution was voided with the operator reason",
    ...
    await clickFilter(page, "Active Contracts");
    await delay(500);
    record("A30-04.13", "register (superseded): Active Contracts excludes the superseded row, 'Superseded (1)' filter shows it, search still finds it; active sum excludes it",
    ...
    record("A30-04.14", "UI re-award reactivates the same agreement (generated) with 'Re-Awarded ... Re-activated' audit and no execution claim",
    ...
    const reAwardSum = regReAward.sum;
    record("A30-04.15", "register after re-award: Pending Execution filter shows the reactivated row; Active Contracted Sum counts it once ($828,000)",
      regReAward.rows.length === 1 && regReAward.sum === "828,000",
      { rows: regReAward.rows.length, sum: regReAward.sum, activeSumBefore: reAwardSum });

    // ---------- 14. responsive probes ----------
    await clickTab(page, "Scope Clash");
    await delay(1200);
    await page.setViewport({ width: 375, height: 780, deviceScaleFactor: 1 });
    await delay(1200);
    const m375 = await page.evaluate(OVERFLOW_PROBE);
    await shot(page, "fix4-qa30-mobile-375-coordination.png", { full: true });
    record("A30-04.16", "coordination tab at 375px: zero page-level horizontal overflow and no unclipped offenders",
      m375.pageOverflow <= 0 && m375.bodyOverflow <= 0 && m375.offenderCount === 0,
      { vw: m375.vw, pageOverflow: m375.pageOverflow, offenderCount: m375.offenderCount, offenders: m375.offenders.slice(0, 4) });

    await clickTab(page, "Subcontracts");
    await delay(1200);
    const m375reg = await page.evaluate(OVERFLOW_PROBE);
    await shot(page, "fix4-qa30-mobile-375-register.png", { full: true });
    record("A30-04.17", "contracts register at 375px: page-level overflow zero (table scrolls inside its own container)",
    ...
    await shot(page, "fix4-qa30-zoom200-coordination.png", { full: true });
    record("A30-04.18", "200% zoom equivalent (720px viewport): register and coordination have zero page-level horizontal overflow",
    ...
    record("A30-04.19", "websocket: reactive connection opened, frames exchanged in both directions, no frame errors, connection stable (no reconnect churn)",
    ...
    record("A30-04.20", "journey diagnostics: zero page errors, zero console errors, no failed app requests",
    ...
    writeEvidence("ui-journey", { results: [...results, { id: "A30-04.ERR", pass: false, name: "aborted", detail: String(err?.stack ?? err) }], reconcile, summary: { pass: results.filter((r) => r.pass).length, total: results.length + 1 } });
