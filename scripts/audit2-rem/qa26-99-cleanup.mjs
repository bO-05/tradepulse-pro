/**
 * QA26-99 cleanup: void any executed QA26 agreements, delete all AUDIT-QA26-*
 * projects, prove foreign projects are untouched.
 */
import { client, writeEvidence, writeLog, sleep } from "./qa26-lib.mjs";
...
async function main() {
  ...
  const foreignBefore = before.filter((p) => !p.title.startsWith("AUDIT-QA26-")).map((p) => `${p.title}|${p._id}`);
  const mine = before.filter((p) => p.title.startsWith("AUDIT-QA26-"));
  const deleted = [];
  const leftovers = [];
    ...
    for (const a of agrs.filter((x) => x.status === "executed")) {
      try {
        await c.mutation("agreements:voidExecutedAgreement", {
          agreementId: a._id,
          reason: "QA26 cleanup: voiding executed agreement before fixture deletion.",
        });
      } catch (err) {
  ...
  const foreignAfter = after.filter((p) => !p.title.startsWith("AUDIT-QA26-")).map((p) => `${p.title}|${p._id}`);
  const stillMine = after.filter((p) => p.title.startsWith("AUDIT-QA26-")).map((p) => p.title);
