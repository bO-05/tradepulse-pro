/**
 * QA27-03 core regression (live, one pass):
...
import { client, readEvidence, writeEvidence, writeLog, projectSnapshot, sleep, call, EVIDENCE_DIR } from "./qa27-lib.mjs";
...
async function main() {
  ...
  const rfiQuestion = "QA27 regression: confirm the firestop sleeve responsibility at the slab penetration detail before bid submission.";
  const rfiSub = await c.mutation("simulation:submitCustomRfi", {
    tradePackageId: S.p2.id,
    contractorId: S.p2.contractors.one,
    subject: "AUDIT-QA27 REGRESSION RFI live smoke",
  ...
  if (progress?.status === "escalated_to_pm") {
    try {
      await c.mutation("rfq:reviewEscalatedRfi", {
        ...
        status: "clarified",
        reviewNote: "QA27 regression: PM certification to complete pending->clarified.",
      });
      certified = true;
  ...
  const dlDir = path.join(EVIDENCE_DIR, "fix4-qa27-smoke-downloads");
  ...
  try {
    ...
    await page.goto(`${BASE}/?project=${S.id}&tab=leveling&qa27=regression`, { waitUntil: "domcontentloaded", timeout: 90000 });
    ...
    await shot(page, "fix4-qa27-inline-confirm.png");
    ...
    await page.evaluate(() => {
      ...
      if (ta) {
        ...
        setter.call(ta, "QA27 DRAFT MARKER");
    ...
    await shot(page, "fix4-qa27-ingest-reopen.png");
    record(
      "REG.modal-reset",
    ...
    await shot(page, "fix4-qa27-deeplink-reload.png");
    record(
      "REG.deeplink-reload",
    ...
    await page.goto(`${BASE}/?project=${S.id}&tab=contracts&qa27=print`, { waitUntil: "domcontentloaded", timeout: 90000 });
    await waitForAppReady(page, 60000);
    await dismissTour(page);
    await delay(2200);
    ...
    await page.goto(`${BASE}/?project=${F.csv.id}&tab=leveling&qa27=csv`, { waitUntil: "domcontentloaded", timeout: 90000 });
    await waitForAppReady(page, 60000);
    await dismissTour(page);
    await delay(1500);
    ...
    await page.goto(`${BASE}/?project=${F.register.id}&tab=contracts&qa27=register`, { waitUntil: "domcontentloaded", timeout: 90000 });
    await waitForAppReady(page, 60000);
    await dismissTour(page);
    await delay(1800);
    ...
    await shot(page, "fix4-qa27-register-superseded.png");
