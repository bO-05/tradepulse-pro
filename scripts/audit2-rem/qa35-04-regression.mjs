/**
 * QA35-04 broad Medium+ regression, one pass (live) on AUDIT-QA35 fixtures:
...
import { client, readEvidence, writeEvidence, writeLog, projectSnapshot, sleep, call, EVIDENCE_DIR, recomputeLeveled, creditInvariants } from "./qa35-lib.mjs";
...
async function main() {
  ...
  const rfiQuestion = "QA35 regression: confirm the firestop sleeve responsibility at the slab penetration detail before bid submission.";
  const rfiSub = await call("F1.rfi.submit", () => c.mutation("simulation:submitCustomRfi", {
    ...
    subject: "AUDIT-QA35 REGRESSION RFI live smoke",
  ...
  const v1 = await call("reg.void.B1", () => c.mutation("agreements:voidExecutedAgreement", { agreementId: a1.value?._id, reason: "QA35 register fixture: execution recorded against superseded pricing, voided for replacement." }));
  ...
  const dlDir = path.join(EVIDENCE_DIR, "fix4-qa35-regression-downloads");
  ...
  try {
    ...
    await page.goto(`${BASE}/?project=${S.id}&tab=leveling&qa35=regression`, { waitUntil: "domcontentloaded", timeout: 90000 });
    ...
    await shot(page, "fix4-qa35-inline-confirm.png");
    ...
    await page.evaluate(() => {
      ...
      if (ta) {
        ...
        setter.call(ta, "QA35 DRAFT MARKER");
    ...
    await shot(page, "fix4-qa35-ingest-reopen.png");
    ...
    await shot(page, "fix4-qa35-deeplink-reload.png");
    ...
    await page.goto(`${BASE}/?project=${S.id}&tab=contracts&qa35=print`, { waitUntil: "domcontentloaded", timeout: 90000 });
    await waitForAppReady(page, 60000);
    await dismissTour(page);
    await delay(2200);
    ...
    await page.goto(`${BASE}/?project=${F.csv.id}&tab=leveling&qa35=csv`, { waitUntil: "domcontentloaded", timeout: 90000 });
    await waitForAppReady(page, 60000);
    await dismissTour(page);
    await delay(1500);
    ...
    await page.goto(`${BASE}/?project=${R.id}&tab=contracts&qa35=register`, { waitUntil: "domcontentloaded", timeout: 90000 });
    await waitForAppReady(page, 60000);
    await dismissTour(page);
    await delay(1800);
    ...
    await shot(page, "fix4-qa35-register-superseded.png");
    ...
    if (progress?.status === "escalated_to_pm") {
      try {
        await c.mutation("rfq:reviewEscalatedRfi", { conversationId: rfiSub.value.conversationId, status: "clarified", reviewNote: "QA35 regression: PM certification to complete pending->clarified." });
