/**
 * QA34-03 full bid-day UI journey on a fresh AUDIT-QA34-JOURNEY project:
...
async function main() {
  ...
  record("A34-03.0", "source claims scan: zero Gemini-3.8 / dedicated-inbox / positive-official-AIA / executed-before-execution strings",
  ...
  for (const p of ((await c.query("projects:listProjects", {})) || []).filter((x) => x.title === TITLE)) {
    ...
    for (const a of agrs0.filter((x) => x.status === "executed")) {
      try { await c.mutation("agreements:voidExecutedAgreement", { agreementId: a._id, reason: "QA34 journey purge of prior run executed record." }); } catch {}
    }
  ...
  try {
    ...
    await page.goto(`${BASE}/?qa34=journey`, { waitUntil: "domcontentloaded", timeout: 90000 });
    ...
    const npFills = {
      ...
      gc: await setVal(page, 'input[aria-label="General contractor or contracting entity"]', "QA34 Journey GC, LLC"),
      ...
      spec: await setVal(page, 'textarea[placeholder*="Outline high-level trade scopes"]', "Division 26 electrical and Division 23 HVAC scope for the QA34 journey."),
    };
    ...
    await delay(2000);
    record("A34-03.1", "fresh project created through the UI dialog", Boolean(proj) && Object.values(npFills).every(Boolean) && npClick.ok, { id: proj?._id });
    ...
    const mk26 = await mkPkgUI("26 00 00", "QA34 Journey Electrical", "900000");
    ...
    const mk23 = await mkPkgUI("23 00 00", "QA34 Journey HVAC", "600000");
    ...
    record("A34-03.2", "Div26 + Div23 packages created through the UI dialog", mk26.go.ok && mk23.go.ok && Boolean(p26) && Boolean(p23) && Object.values(mk23.filled).every(Boolean), { p26: p26?._id, p23: p23?._id });
    ...
    const ctrGo1 = await addCtr(p26.tradeName, "AUDIT-QA34 Journey Prime", "estimating@qa34-journey-e.invalid", "HI-QA34-JE");
    const ctrGo2 = await addCtr(p23.tradeName, "AUDIT-QA34 Journey Mechanical", "estimating@qa34-journey-m.invalid", "HI-QA34-JM");
    ...
    const ctr26 = (ctrs26 || []).find((x) => /QA34 Journey Prime/.test(x.companyName));
    const ctr23 = (ctrs23 || []).find((x) => /QA34 Journey Mechanical/.test(x.companyName));
    record("A34-03.3", "contractors added through the UI for both packages", ctrGo1.ok && ctrGo2.ok && Boolean(ctr26) && Boolean(ctr23), { c26: ctr26?.companyName, c23: ctr23?.companyName });
    ...
    record("A34-03.4", "bids submitted (backend submitDirectBid; deterministic direct-bid UI does not exist, AI ingest excluded)", bid26.leveledTotalCost === 800000 && bid23.leveledTotalCost === 470000, { bid26, bid23 });
    ...
    record("A34-03.5", "cross-trade pre-state: UI KPI == backend (buys $50,500, voids $46,500, credits $0, no chips/controls)",
    ...
    record("A34-03.6", "UI clash scan banner reconciles with the backend scan message", scanTry.ok && Boolean(scanMsg) && scanBackend?.analyzed === true && scanMsg === scanBackend.message, { uiMessage: scanMsg, backendMessage: scanBackend?.message ?? null });
    ...
    record("A34-03.7", "UI 1-click deduct: HVAC -$38,500; card/KPI reconcile (credits $38,500, buys $12,000); reverse control visible",
    ...
    record("A34-03.8", "UI reverse from the card: HVAC restored to $470,000, credit row removed, card detected $38,500, KPI credits $0 / buys $50,500, audit says restored $470,000",
    ...
    record("A34-03.9", "UI re-deduct after reverse applies exactly once (431,500; one credit row; credits $38,500)",
    ...
    await delay(1200);
    const adjustClick = await clickRowButton(page, "QA34 Journey Mechanical", "Adjust");
    await delay(900);
    ...
    record("A34-03.10", "stale created through the real leveling Adjust modal: credit row declined via UI toggle + Save; bid back to $470,000",
    ...
    record("A34-03.11", "stale overlay truth: card detected $38,500 with staleResolution, 'Clear stale credit record' visible, KPI credits $0 / buys $50,500",
    ...
    record("A34-03.12", "refusal probe: deduct while stale refuses with a toast ('already been applied'), bid untouched, ZERO pageerrors from the refusal",
    ...
    const clearToast = await poll(() => toastText(page), (t) => Boolean(t) && /reversed/.test(t), 12000, 400);
    await delay(1200);
    ...
    record("A34-03.13", "UI 'Clear stale credit record': declined row removed, bid stays $470,000, card fully detected (no stale), toast confirms",
    ...
    record("A34-03.14", "re-deduct after stale clear recovers fully: 431,500; credits $38,500; one reverse control",
    ...
    record("A34-03.15", "UI assign scope void: Div26 +$28,000, inclusion added, KPI voids $18,500, assigned chip 1",
    ...
    record("A34-03.16", "UI award generates the agreement at 828,000; audit says 'generated ... pending external execution'",
    ...
    record("A34-03.17", "UI execution: executed status; viewer flips from Pending Execution to RECORDED • SIGNATURE REQUIRED; audit avoids a 'fully executed' claim",
    ...
    record("A34-03.18", "register (executed): chip 1/1, row badge, search by agreement number finds the row, Pending filter empty",
    ...
    record("A34-03.19", "UI void: agreement superseded, bid unawarded; viewer flips to Superseded; audit records the operator reason",
    ...
    await clickFilter(page, "Active Contracts");
    await delay(500);
    record("A34-03.20", "register (superseded): Active excludes the voided row and sum is 0; Superseded filter shows it read-only; search still finds it",
    ...
    record("A34-03.21", "UI re-award reactivates the same agreement (generated) with 'Re-Awarded ... Re-activated' audit and no execution claim",
    ...
    record("A34-03.22", "register after re-award: Pending Execution shows the reactivated row once; Active Contracted Sum counts it once ($828,000)",
    ...
    record("A34-03.23", "live claims sweep across all 8 tabs: zero forbidden claims (Gemini-3.8 / dedicated inbox / positive official AIA / executed-before-execution / static scan fallback)",
    ...
    await shot(page, "fix4-qa34-mobile-375-coordination.png", { full: true });
    ...
    record("A34-03.24", "coordination at 375px (credit applied, reverse control rendered): zero page-level horizontal overflow, no unclipped offenders",
    ...
    await shot(page, "fix4-qa34-mobile-375-register.png", { full: true });
    await page.setViewport({ width: 720, height: 900, deviceScaleFactor: 1 });
    await delay(1200);
    const m720reg = await page.evaluate(OVERFLOW_PROBE);
    await shot(page, "fix4-qa34-zoom200-register.png", { full: true });
    await clickTab(page, "Scope Clash");
    await delay(1200);
    const m720coord = await page.evaluate(OVERFLOW_PROBE);
    await shot(page, "fix4-qa34-zoom200-coordination.png", { full: true });
    record("A34-03.25", "200% zoom equivalent (720px): register and coordination zero page-level horizontal overflow",
    ...
    record("A34-03.26", "websocket: reactive connection opened, frames exchanged both ways, no frame errors",
    ...
    record("A34-03.27", "journey diagnostics: ZERO page errors (including the stale refusal), no failed app requests; the only console error is the Convex client logging the expected refused mutation",
    ...
    writeEvidence("ui-journey", { results: [...results, { id: "A34-03.ERR", pass: false, name: "aborted", detail: String(err?.stack ?? err) }], reconcile, summary: { pass: results.filter((r) => r.pass).length, total: results.length + 1 } });
